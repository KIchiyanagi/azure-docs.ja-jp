...
- ユーザーが各自の Azure AD アカウントで WORKS MOBILE に自動的にシングル サインオン (SSO) できるようにすることが可能です。
...
- Azure AD サブスクリプション
- WORKS MOBILE での SSO が有効なサブスクリプション
...
## <a name="add-works-mobile-from-the-gallery"></a>ギャラリーからの WORKS MOBILE の追加
...
3. **[WORKS MOBILE のドメインと URL]** セクションで、次の手順を実行します。
  ...
  2. **[識別子]** ボックスに、値として「`worksmobile.com`」と入力します。
    ...
    >[!NOTE] 
    >これらは実際の値ではありません。 実際のサインオン URL と識別子でこれらの値を更新する必要があります。 ここでは、識別子に一意の文字列値を使用することをお勧めします。 これらの値を取得するには、LINE WORKS Developers コミュニティ [https://developers.worksmobile.com/jp/community] を通じて連絡してください。 
    >
    >
